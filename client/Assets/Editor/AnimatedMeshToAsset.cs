...
public class AnimatedMeshToAsset
{
    ...
    private static void Generate()
    {
        ...
        Debug.Log(path);
        var selectionPath = Path.GetDirectoryName(path);
    ...
    private static Texture GenerateAnimationTexture(GameObject targetObject, IEnumerable<AnimationClip> clips, SkinnedMeshRenderer smr)
{
    ...
    var initialRootPosition = rootBone.localPosition;
    ...
    foreach (var clip in clips)
    {
        ...
        foreach (var frame in Enumerable.Range(0, totalFrames))
        {
            ...
            // 关键修改点：仅重置根骨骼的本地位置（保留旋转和缩放）
            rootBone.localPosition = initialRootPosition;

            // 强制更新骨骼变换（确保后续矩阵计算正确）
            foreach (var bone in smr.bones)
            {
                var _ = bone.localToWorldMatrix; // 访问属性以强制刷新
            }
